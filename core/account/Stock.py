...
from utils.Constants import OrderSide, DefaultAccountType
...
class StockAccount(BaseAccount):
    ...
    def get_state(self):
        return {
            'positions': {
                ...
                for order_book_id, position in self._positions.items()
            },
    ...
    def set_state(self, state):
        ...
        for order_book_id, v in state['positions'].items():
    ...
    def _handle_dividend_payable(self, trading_date):
        to_be_removed = []
        for order_book_id, dividend in self._dividend_receivable.items():
    ...
    def dividend_receivable(self):
        ...
        """
        return sum(d['quantity'] * d['dividend_per_share'] for d in self._dividend_receivable.values())
